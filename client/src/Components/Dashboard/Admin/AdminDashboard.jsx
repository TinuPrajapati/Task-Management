...
const AdminDashboard = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${backendUrl}/admin/all_tasks`, {
        headers: {
          Authorization: `Bearer ${Cookies.get(import.meta.env.VITE_cookies_name)}`,
        },
      });
    ...
    <div className="w-[100vw] h-[100vh] bg-gray-900 px-8 py-5 text-white flex flex-col gap-6">
      ...
      <div className="w-full h-[80%] bg-slate-600 py-2 rounded-lg flex flex-col gap-2">
        <div className="w-full h-[15%] flex justify-between items-center font-serif text-2xl px-10 border-b-2 py-2">
          ...
          <div
            className="bg-gradient-to-r from-violet-500 to-fuchsia-500 h-full w-[15%] rounded-md flex justify-center items-center flex-col relative"
          ...
          >
            ...
            <div
              className={`${
                display ? "flex" : "hidden"
              } absolute top-[102%] flex-col w-full bg-white justify-center items-center gap-2 py-2 rounded-md`}
            >
              {/* All Task option */}
              <Link
                to={`/admin/${name}/all_tasks`}
                className="w-[80%] h-10 bg-gradient-to-b from-pink-400 via-purple-500 to-indigo-600 flex justify-center items-center rounded-md active:scale-90 duration-200"
              >
                All Task
              </Link>
              {/* All Users option */}
              <Link
                to={`/admin/${name}/all_users`}
                className="w-[80%] h-10 bg-gradient-to-b from-pink-400 via-purple-500 to-indigo-600 flex justify-center items-center rounded-md active:scale-90 duration-200"
              >
                All Users
              </Link>
        ...
        <div className="w-full h-[85%] flex justify-evenly items-center gap-4 px-4 py-8">
          <Link to={`/admin/${name}/all_tasks`} className="w-[25%] h-full">
            <Task
              task="Pending Task"
              color="bg-blue-500"
